...
* O vínculo com Tarefas pode ser feito de três formas, todas equivalentes (mesmo vínculo N:N, mesmo resultado): (1) no momento do cadastro do requisito, selecionando uma ou mais tarefas no modal de criação (`task_ids`, seção 3.2); (2) editando o requisito (`POST /web/requirements/{requirement_id}`, seção 3.2) — o modal de edição envia o conjunto completo de `task_ids` marcados, e o servidor sincroniza os vínculos (vincula os novos, desvincula os que foram desmarcados); ou (3) posteriormente, pela tabela de Requisitos, uma tarefa por vez (`POST /web/requirements/{requirement_id}/tasks`, com `action=add`/`remove`).
* O mesmo vale para o vínculo com Atores (seção 2.7): a edição do requisito (`actor_ids`) sincroniza os vínculos da mesma forma que a criação e a tabela de Requisitos.
...
* **Recursos visuais:**
  ...
  * **Modais Interativos:**
    ...
    * Modal de Edição de Requisito (`modal-lg`; título, descrição, tipo e prioridade; `code` é exibido somente leitura, pois é imutável após o cadastro; inclui também as listas de checkboxes de Tarefas Vinculadas e Atores Vinculados — marcar ou desmarcar um item na edição vincula ou desvincula a tarefa/ator correspondente ao salvar, ver seção 2.6).
    * Modal de Visualização de Requisito (somente leitura): clicar no `code` de um requisito, na tabela de Requisitos, abre uma modal com o **mesmo layout do formulário de edição** (`modal-lg`; código, título, descrição, tipo, prioridade, status e as mesmas listas de Tarefas/Atores vinculados), porém com todos os campos desabilitados e sem o botão de salvar — o rodapé tem apenas um botão **Voltar**, que fecha a modal sem gravar nada; serve para consulta rápida do requisito completo sem sair do dashboard.
...
* **Editar Requisito:** `POST /web/requirements/{requirement_id}` (Campos opcionais: `title`, `description`, `type`, `priority` — atualização parcial, igual à edição de tarefa; `code` não é aceito, pois é imutável — ver RN-08). Também aceita `task_ids` e `actor_ids` (múltiplos valores, um por checkbox marcado): ao contrário dos demais campos, esses dois **não são parciais** — o servidor sincroniza os vínculos do requisito para corresponder exatamente ao conjunto enviado (vincula os ausentes, desvincula os que não vieram no POST); omitir um dos campos equivale a enviar uma lista vazia, removendo todos os vínculos daquele tipo (ver seção 2.6).
