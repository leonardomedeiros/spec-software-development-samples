# Especificação Técnica: API RESTful de Gestão de Tarefas (TaskTrack)
...
### 1.1 Objetivo
O **TaskTrack** é um microsserviço RESTful responsável pelo gerenciamento de projetos e suas respectivas tarefas. O sistema permite a criação de projetos, cadastro de tarefas com prioridades e prazos, atribuição de usuários responsáveis e controle rigoroso do ciclo de vida dos status das tarefas.

### 1.2 Stack Tecnológica
* **Linguagem:** Python 3.11+
* **Framework Web:** FastAPI (com suporte nativo a `asyncio` e Pydantic v2)
* **Persistência / ORM:** PostgreSQL 15+ com SQLAlchemy 2.0 (modo assíncrono) e Alembic para migrações
* **Validação de Dados:** Pydantic v2
* **Containerização:** Docker e Docker Compose

### 1.3 Arquitetura e Restrições
* **Padrão Arquitetural:** *Clean Architecture* (Arquitetura Limpa), dividida nas seguintes camadas:
  1. `Domain`: Entidades puras, objetos de valor e exceções de domínio.
  2. `Use Cases`: Regras de negócio da aplicação e orquestração.
  3. `Adapters / Controllers`: Controllers FastAPI, Schemas DTO e Presenters.
  4. `Infrastructure`: Repositórios SQLAlchemy, integração com Banco de Dados e configurações externas.
* **Injeção de Dependências:** Deve-se utilizar a injeção de dependência nativa do FastAPI (`Depends`).
* **Restrição de Camadas:** É proibido importar frameworks de infraestrutura (FastAPI, SQLAlchemy) dentro do módulo `Domain`.
...
## 3. Contratos de API / Interface

### 3.1 `POST /api/v1/projects` — Criar Projeto
...
### 3.2 `POST /api/v1/projects/{project_id}/tasks` — Criar Tarefa
...
### 3.3 `PATCH /api/v1/tasks/{task_id}/status` — Atualizar Status da Tarefa
...
## 6. Ferramentas e Modelos Recomendados

### 6.1 LLMs Recomendados para Geração de Código
1. **Claude 3.5 Sonnet:** Recomendado para gerar o código-fonte seguindo Clean Architecture em Python, com alta fidelidade às regras de tipagem estática e Pydantic v2.
2. **GPT-4o:** Excelente para criar as rotas FastAPI, migrações Alembic e montar a suíte de testes automatizados com `pytest`.
3. **Gemini 1.5 Pro:** Indicado para refatorações de código completo e análise de coerência entre contratos de API e schemas DDL.

### 6.2 Ferramentas de Desenvolvimento e Validação
* **Linter & Formatting:** `ruff` e `black`
* **Análise Estática de Tipos:** `mypy --strict`
* **Testes e Cobertura:** `pytest`, `pytest-asyncio`, `httpx` (para chamadas de API assíncronas) e `coverage`
* **Documentação Viva:** Swagger UI / OpenAPI (gerado automaticamente pelo FastAPI no endpoint `/docs`)
